...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(jwtPayload: JWTSignPayload): Promise<User> {
    ...
    const signed: any | JWTSignPayload = await this.jwtService.decode(user.auth);
    if (signed.__id != jwtPayload.__id) throw new ExpiredTokenException(EXPIRED_AUTH_TOKEN);
